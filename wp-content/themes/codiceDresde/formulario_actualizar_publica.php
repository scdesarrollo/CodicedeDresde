...
/*---------------------------------------------------------------------------------------------*\
   AJAX INSERTAR AUDIO
\*---------------------------------------------------------------------------------------------*/
if( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) &&  $_POST['action'] == "update_post_publica") {
...
// Do some minor form validation to make sure there is content
    ...
    if (isset ($_POST['id_valor_hp'])) {
        $id_valor_hp =  $_POST['id_valor_hp'];
    } 
...
// Update post 37
  $my_post = array(
      ...
      'tags_input'    =>  $id_valor_hp . "," . $tags_txt,
  ...
  $pid = wp_update_post($my_post);
	...
	if ( $pid ) {
		echo "Los datos publicos se actualizaron correctamente...";
    }

 update_post_meta( $pid , "id_correo", "$correo_nota");
 update_post_meta( $pid , "id_tipoNota", "$tipo_nota");
